...
# Adjusted imports for local copy
...
class MeshRender():
    def __init__(
        ...
        if self.raster_mode == 'cr':
            # Assuming custom_rasterizer is installed in the environment
            try:
                import custom_rasterizer as cr
                self.raster = cr
            except ImportError as e:
                print(f"ERROR: Could not import custom_rasterizer. Please ensure it's installed correctly.")
                print(f"Refer to ComfyUI-Hunyuan3DWrapper README for installation instructions.")
                raise e
        else:
    ...
    def raster_texture(self, tex, uv, uv_da=None, mip_level_bias=None, mip=None, filter_mode='auto',
        ...
        if self.raster_mode == 'cr':
            # This was the source of the previous error - texture sampling needs to be manual
            # raise f'Texture is not implemented in cr'
            # Perform manual sampling using F.grid_sample instead
            tex_for_sampling = tex.permute(0, 3, 1, 2) # Shape (B, C, H, W)
            coords = uv * 2.0 - 1.0 # Convert coords from [0, 1] to [-1, 1]
            sample_mode = 'bilinear' if filter_mode == 'linear' else 'nearest'
            color = F.grid_sample(tex_for_sampling, coords, mode=sample_mode, padding_mode='border', align_corners=False)
            color = color.permute(0, 2, 3, 1) # Shape (B, H, W, C)
            return color
        else:
            raise f'No raster named {self.raster_mode}'

        # return color # This line was unreachable
    ...
    def load_mesh(
        ...
        # Use the texture_data returned by the load_mesh utility function
        if texture_data is not None:
            print("### DEBUG: Texture data found by utility, calling self.set_texture...") # Debug print
            self.set_texture(texture_data)
        # Fallback if no texture found by utility and self.tex is not already set
        elif self.tex is None:
             print("WARN: No texture found by utility or already set, creating dummy texture.")
             # Create a dummy grey texture if none exists
             dummy_tex_np = np.full((self.texture_size[0], self.texture_size[1], 3), 0.5, dtype=np.float32)
             self.tex = torch.from_numpy(dummy_tex_np).to(self.device).float()
    ...
    def set_mesh(
        ...
        if (vtx_uv is not None) and (uv_idx is not None):
            ...
            # Create default UVs if none exist (simple planar projection)
            print("WARN: No UVs found on input mesh, creating default planar UVs.")
            self.vtx_uv = self.vtx_pos[:, :2].clone() # Use XY coordinates
            self.vtx_uv = (self.vtx_uv - self.vtx_uv.min(0, keepdim=True)[0]) / (self.vtx_uv.max(0, keepdim=True)[0] - self.vtx_uv.min(0, keepdim=True)[0])
            self.uv_idx = self.pos_idx # Use same indices as positions

        self.vtx_pos[:, [0, 1]] = -self.vtx_pos[:, [0, 1]]
        self.vtx_pos[:, [1, 2]] = self.vtx_pos[:, [2, 1]]
        if (vtx_uv is not None) and (uv_idx is not None): # Check again as we might have created defaults
        ...
        if auto_center:
            ...
            # Avoid division by zero if scale is zero (e.g., single point)
            if scale > 1e-8:
                 self.vtx_pos = (self.vtx_pos - center) * (scale_factor / float(scale))
            else:
                 self.vtx_pos = self.vtx_pos - center # Just center it
            self.scale_factor = scale_factor

    def set_texture(self, tex):
        if isinstance(tex, np.ndarray):
            # Assuming input is HWC, 0-255 uint8
            tex = Image.fromarray(tex.astype(np.uint8))
        elif isinstance(tex, torch.Tensor):
            # Assuming input is B H W C, 0-1 float
            tex = tex.squeeze(0).cpu().numpy()
            tex = Image.fromarray((tex * 255).astype(np.uint8))
        # If not PIL Image, ndarray, or tensor, assume it's PIL Image already
        elif not isinstance(tex, Image.Image):
             raise TypeError(f"Unsupported texture type: {type(tex)}")

        # --- Removed resize step to preserve original texture resolution ---
        # tex = tex.resize(self.texture_size).convert('RGB')
        tex = tex.convert('RGB') # Ensure RGB format
        # --- Update self.texture_size based on the actual loaded texture ---
        self.set_default_texture_resolution(tex.size) # tex.size is (width, height)
        print(f"### DEBUG set_texture: Set internal texture_size to {self.texture_size}")
    ...
    def _render(
        ...
        # Correct call to raster_rasterize (removed glctx)
        rast_out, rast_out_db = self.raster_rasterize(
            pos_clip, pos_idx, resolution=resolution)

        tex = tex.contiguous()
        if filter_mode == 'linear-mipmap-linear':
            # Mipmapping not supported with grid_sample, ignore texd
            texc, _ = self.raster_interpolate(
                 uv[None, ...], rast_out, uv_idx, rast_db=rast_out_db, diff_attrs='uv')
            # Manual texture sampling using grid_sample
            tex_for_sampling = tex.permute(2, 0, 1).unsqueeze(0) # Shape (1, C, H, W)
            coords = texc * 2.0 - 1.0 # Convert coords from [0, 1] to [-1, 1]
            color = F.grid_sample(tex_for_sampling, coords, mode='bilinear', padding_mode='border', align_corners=False)
            color = color.permute(0, 2, 3, 1) # Shape (1, H, W, C)

        else: # Assuming 'linear' or 'nearest'
             texc, _ = self.raster_interpolate(uv[None, ...], rast_out, uv_idx)
             # Manual texture sampling using grid_sample
             tex_for_sampling = tex.permute(2, 0, 1).unsqueeze(0) # Shape (1, C, H, W)
             coords = texc * 2.0 - 1.0 # Convert coords from [0, 1] to [-1, 1]
             # Use appropriate mode based on filter_mode
             sample_mode = 'bilinear' if filter_mode == 'linear' else 'nearest'
             color = F.grid_sample(tex_for_sampling, coords, mode=sample_mode, padding_mode='border', align_corners=False)
             color = color.permute(0, 2, 3, 1) # Shape (1, H, W, C)


        visible_mask = torch.clamp(rast_out[..., -1:], 0, 1) # Shape (1, H, W, 1)
        color = color * visible_mask # Apply mask. Shape (1, H, W, C)
        if self.use_antialias:
            color = self.raster_antialias(color, rast_out, pos_clip, pos_idx)

        if keep_alpha: # Use argument again
    ...
    def render(
        ...
        filter_mode=None,
        return_type='th',
        pan_x=0.0, # Added pan_x
        pan_y=0.0  # Added pan_y
    ):
        ...
        # Use self.tex if tex argument is None
        tex_to_use = self.tex if tex is None else tex
        print(f"### DEBUG: Texture in render method (self.tex): {type(self.tex)}, shape: {self.tex.shape if self.tex is not None else 'None'}") # Debug print
        if tex_to_use is None:
             # Handle case where no texture is loaded or provided
             print("WARN: No texture available for rendering (tex_to_use is None). Returning background color.")
             if resolution is None: resolution = self.default_resolution
             if isinstance(resolution, int): resolution = (resolution, resolution)
             if bgcolor is None: bgcolor = [0,0,0] # Default to black if no bgcolor
             image = torch.tensor(bgcolor, device=self.device, dtype=torch.float32).view(1, 1, 3).repeat(resolution[0], resolution[1], 1)
             mask = torch.zeros(resolution[0], resolution[1], 1, device=self.device, dtype=torch.float32) # No mask
        else:
            if isinstance(tex_to_use, Image.Image):
                tex_to_use = torch.tensor(np.array(tex_to_use) / 255.0)
            elif isinstance(tex_to_use, np.ndarray):
                tex_to_use = torch.tensor(tex_to_use)
            if tex_to_use.dim() == 2:
                tex_to_use = tex_to_use.unsqueeze(-1)
            tex_to_use = tex_to_use.float().to(self.device)

            # Use keyword arguments explicitly for the _render call
            image = self._render(
                glctx=self.raster,
                mvp=r_mvp,
                pos=self.vtx_pos,
                ...
                uv=self.vtx_uv,
                uv_idx=self.uv_idx,
                tex=tex_to_use, # Use the determined texture
                resolution=(self.default_resolution if resolution is None else resolution),
                max_mip_level=self.max_mip_level,
                keep_alpha=True, # Keep alpha internally for masking
                filter_mode=self.filter_mode
            )
            mask = (image[..., [-1]] == 1).float()
            if bgcolor is None:
                bgcolor = [0 for _ in range(image.shape[-1] - 1)] # Default based on image channels
            image = image * mask + (1 - mask) * \
                    torch.tensor(bgcolor + [0], device=self.device, dtype=torch.float32) # Add alpha to bgcolor

        if keep_alpha == False:
            image = image[..., :-1] # Remove alpha if not requested

        if return_type == 'np':
            image = image.cpu().numpy()
        elif return_type == 'pl':
            # Ensure 3 channels (RGB) before converting to PIL
            if image.shape[-1] == 4:
                 image = image[..., :3]
            image = (image.cpu().numpy() * 255).astype(np.uint8)
            image = Image.fromarray(image)
        # Default return_type='th'
        elif image.shape[-1] == 4 and not keep_alpha:
             image = image[..., :3] # Ensure 3 channels if alpha not kept
    ...
    def render_normal(
        ...
        # Only normalize tangent space normals to [0, 1] range for visualization
        if normalize_rgb and not use_abs_coor:
            normal = (normal + 1) * 0.5
        # Removed incorrect clamp for world normals (use_abs_coor=True)
        # elif use_abs_coor:
        #     normal = torch.clamp(normal, min=0.0)
    ...
    def render_depth(
        ...
        # Avoid division by zero if depth range is zero
    ...
    def render_position(self, elev, azim, camera_distance=None, center=None,
                        ...
                        pan_x=0.0, pan_y=0.0): # Added pan_x and pan_y
        pos_camera, pos_clip = self.get_pos_from_mvp(elev, azim, camera_distance, center, pan_x=pan_x, pan_y=pan_y) # Pass pan values
        if resolution is None:
            resolution = self.default_resolution
        if isinstance(resolution, (int, float)):
            resolution = [resolution, resolution]
    ...
    def back_project(self, image, elev, azim,
                     ...
                     pan_x=0.0, pan_y=0.0): # Added pan_x, pan_y
        if isinstance(image, Image.Image):
        ...
        image = image.float().to(self.device) # This is the low-res inpainted view (e.g., 1024x1024)
        resolution = image.shape[:2] # --- REVERT: Use input image resolution for rasterization ---
        channel = image.shape[-1]
        texture = torch.zeros(self.texture_size + (channel,)).to(self.device) # Target high-res texture
        cos_map = torch.zeros(self.texture_size + (1,)).to(self.device) # Target high-res cos map
        ...
        vertex_normals = torch.from_numpy(
            vertex_normals).float().to(self.device).contiguous()
        tex_depth = pos_camera[:, 2].reshape(1, -1, 1).contiguous()
        # --- Rasterize at INPUT resolution ---
        rast_out, rast_out_db = self.raster_rasterize(
            pos_clip, self.pos_idx, resolution=resolution) # resolution is input image size
        visible_mask = torch.clamp(rast_out[..., -1:], 0, 1)[0, ...] # Low-res visible mask

        # --- Interpolate attributes at INPUT resolution ---
        normal, _ = self.raster_interpolate(
            vertex_normals[None, ...], rast_out, self.pos_idx)
        normal = normal[0, ...] # Low-res normals
        uv, _ = self.raster_interpolate(self.vtx_uv[None, ...], rast_out, self.uv_idx) # Low-res UVs (B, H, W, 2)
        depth, _ = self.raster_interpolate(tex_depth, rast_out, self.pos_idx)
        depth = depth[0, ...] # Low-res depth
        ...
        if method == 'linear':
            proj_mask = (visible_mask != 0).view(-1) # Low-res mask
            uv = uv.squeeze(0).contiguous().view(-1, 2)[proj_mask] # Low-res UVs for visible pixels
            image = image.squeeze(0).contiguous().view(-1, channel)[proj_mask] # Low-res image data for visible pixels
            cos_image = cos_image.contiguous().view(-1, 1)[proj_mask] # Low-res cos values for visible pixels
            sketch_image = sketch_image.contiguous().view(-1, 1)[proj_mask] # Low-res sketch values for visible pixels

            # --- Project low-res data onto HIGH-RES texture map ---
            texture = linear_grid_put_2d(
                self.texture_size[1], self.texture_size[0], uv[..., [1, 0]], image) # Target H, W = self.texture_size
            cos_map = linear_grid_put_2d(
                self.texture_size[1], self.texture_size[0], uv[..., [1, 0]], cos_image) # Target H, W = self.texture_size
